...
class CallingV3Tests : IntegrationTestBase {
    ...
    func otherStartCall(user: ZMUser, isVideoCall: Bool = false, shouldRing: Bool = true) {
        ...
        zmessaging.IncomingCallHandler(conversationId: conversationIdRef, userId: userIdRef, isVideoCall: isVideoCall ? 1 : 0, shouldRing: shouldRing ? 1 : 0, contextRef: wireCallCenterRef)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
    }
    ...
    func otherJoinCall(user: ZMUser) {
        ...
        if useGroupConversation {
            ...
            zmessaging.AnsweredCallHandler(conversationId: conversationIdRef, contextRef: wireCallCenterRef)
            XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
        }
    }
    ...
    func establishedFlow(user: ZMUser){
        ...
        let userIdRef = user.remoteIdentifier!.transportString().cString(using: .utf8)
        zmessaging.EstablishedCallHandler(conversationId: conversationIdRef, userId: userIdRef, contextRef: wireCallCenterRef)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
    }
    ...
    func participantsChanged(user: ZMUser, establishedFlow: Bool) {
        ...
        zmessaging.GroupMemberHandler(conversationIdRef: conversationIdRef, callMembersRef: &members, contextRef: wireCallCenterRef)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
    ...
    func closeCall(user: ZMUser, reason: CallClosedReason) {
        ...
        let userIdRef = user.remoteIdentifier!.transportString().cString(using: .utf8)
        zmessaging.ClosedCallHandler(reason: reason.rawValue, conversationId: conversationIdRef, userId: userIdRef, metrics: nil, contextRef: wireCallCenterRef)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
    }
    ...
    func simulateMissedCall(user: ZMUser) {
        ...
        let userIdRef = user.remoteIdentifier!.transportString().cString(using: .utf8)
        zmessaging.MissedCallHandler(conversationId: conversationIdRef, messageTime: UInt32(Date().timeIntervalSince1970), userId: userIdRef, isVideoCall: 0, contextRef: wireCallCenterRef)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
    }
